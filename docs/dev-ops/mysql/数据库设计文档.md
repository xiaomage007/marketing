...
| `raffle_activity_account` | **混合** | 路由键运营写入(`user_id`、`activity_id`);`total_count`/`day_count`/`month_count` 取自 `raffle_activity_count` 的额度配置(首次落库时由流程写入);`total_count_surplus`/`day_count_surplus`/`month_count_surplus` 由流程扣减/重置 |

> **公共字段说明**:`create_time`、`update_time` 在所有表中均由数据库/系统自动维护,不计入业务字段分类。

#### 6.6.2 数据流转场景中的字段来源标注

把 6.1 ~ 6.4 四个场景中,涉及的每一个字段的来源都明确标注,便于一眼区分"运营做了什么"和"系统做了什么"。
...
| 账户额度 total_count day_count month_count | 流程生成 | 从 raffle_activity_count 拷贝,首次落库时写入 |
| 账户剩余 total_count_surplus day_count_surplus month_count_surplus | 流程生成 | 初始化为额度值 |
...
| 账户 surplus 系列扣减 | 流程生成 | raffle_activity_account UPDATE |
...
| 用户已抽奖次数 实时统计 | 流程生成 | SELECT COUNT FROM raffle_activity_order_xxx |
...
| 订单字段不可变 | `raffle_activity_order` 全部字段均为流程生成,流水性质,设计上不允许运营修改;`activity_name`、`strategy_id` 是冗余字段,即使运营改了 `raffle_activity`,历史订单保持原值不变 |
