...
> 更新日期：2026-09-16
> 对应分支：`260915-Charlie-user-credit-account`（第 25 节：积分发奖服务实现）
> 配套文档：`docs/营销抽奖系统 详细设计文档.md`
...
- **字符集**：`utf8mb4`
- **存储引擎**：`InnoDB`
- **总表数**：11 张（公共库）+ 42 张（分库物理表：4 种分表 × 4 张 × 2 库 + 5 种不分表 × 2 库），合计 53 张
...
marketing（公共库，不拆分）
  ...
  └── daily_behavior_rebate    ← 日常行为返利配置
...
marketing_01（分库 1，按 userId 哈希路由）          marketing_02（分库 2，按 userId 哈希路由）
  ...
  ├── task                               ← 不分表        ├── task                               ← 不分表
  ├── user_credit_account               ← 不分表（第 25 节新增） ├── user_credit_account       ← 不分表（第 25 节新增）
  ...
  ├── user_behavior_rebate_order_000    ← 分表 0        ├── user_behavior_rebate_order_000    ← 分表 0
  ├── user_behavior_rebate_order_001    ← 分表 1        ├── user_behavior_rebate_order_001    ← 分表 1
  ├── user_behavior_rebate_order_002    ← 分表 2        ├── user_behavior_rebate_order_002    ← 分表 2
  └── user_behavior_rebate_order_003    ← 分表 3        └── user_behavior_rebate_order_003    ← 分表 3
```

- **路由键**：`userId`
- **分库数**：2（`marketing_01`、`marketing_02`）
- **分表数**：`raffle_activity_order` / `user_raffle_order` / `user_award_record` / `user_behavior_rebate_order` 每库 4 张（`_000` ~ `_003`），其余账户表（`raffle_activity_account` 系列、`user_credit_account`）与 `task` 表每库 1 张（不分表）
...
erDiagram
    ...
    daily_behavior_rebate ||--o{ user_behavior_rebate_order : "behavior_type"
    user_award_record ||--o| user_credit_account : "user_id 发奖入账（软关联）"
```

### 2.2 公共库 ER 图（marketing）

> 包含 11 张公共表（`award`、`strategy`、`strategy_award`、`strategy_rule`、`raffle_activity`、`raffle_activity_count`、`raffle_activity_sku`、`rule_tree`、`rule_tree_node`、`rule_tree_node_line`、`daily_behavior_rebate`）及其字段、约束与外键关系。

![公共库ER图](er_diagram_public.png)

### 2.3 分库分表 ER 图（marketing_01 / marketing_02）

> `raffle_activity_order` / `user_raffle_order` / `user_award_record` / `user_behavior_rebate_order` 按 `userId` 哈希分到 `marketing_01` / `marketing_02`，每个库内再分 4 张表（`_000` ~ `_003`）；`raffle_activity_account` / `raffle_activity_account_day` / `raffle_activity_account_month` / `task` / `user_credit_account` 分库但不分表。
...
| **积分账户域** | `user_credit_account` | 用户积分账户（总积分/可用积分），发奖消费侧（`SendAwardCustomer` → `UserCreditRandomAward` / `UserCreditBlacklistAward`）累加入账（**第 25 节已落地**） |
| **任务补偿域** | `task` | MQ 消息落库表（含 RabbitMQ 投递参数），用于跨库分布式事务的最终一致性补偿 |
| **规则树域** | `rule_tree`、`rule_tree_node`、`rule_tree_node_line` | 可编排的决策树，通过连线控制节点流转 |
| **行为返利域** | `daily_behavior_rebate`、`user_behavior_rebate_order` | 日常行为（签到/支付）返利配置 + 用户返利入账流水，返利可为 sku 充值或积分 |
...
**索引**：`PRIMARY KEY (id)`、`UNIQUE KEY uq_award_id (award_id)`（第 25 节新增，发奖侧按 `award_id` 查 `award_key` / `award_config` 时的唯一性保障）
...
| 100003 | 抽奖策略-验证lock | NULL | 无规则（原仅黑名单，已移除） |
...
| 102 | OpenAI会员卡 | 0.0300 | 100 | `tree_luck_award` | — |
| 103 | 支付优惠券 | 0.0300 | 100 | `tree_luck_award` | — |
| 104 | 小米台灯 | 0.0300 | 100 | `tree_luck_award` | — |
| 105 | 小米su7周体验 | 0.0300 | 100 | `tree_lock_3` | 抽3次后解锁 |
| 106 | 轻奢办公椅 | 0.0300 | 100 | `tree_lock_2` | 抽2次后解锁 |
| 107 | 小霸王游戏机 | 0.0300 | 100 | `tree_lock_1` | 抽1次后解锁 |
| 108 | 暴走玩偶 | 0.0300 | 100 | `tree_luck_award` | — |
...
**索引**：`PRIMARY KEY (id)`、`UNIQUE KEY uq_activity_id (activity_id)`、`UNIQUE KEY uq_strategy_id (strategy_id)`、`KEY idx_begin_date_time (begin_date_time)`、`KEY idx_end_date_time (end_date_time)`

> `uq_strategy_id` 为"活动-策略"一对一约束：保证一个策略在同一时间只能被一个活动使用，避免多活动复用同一策略导致抽奖资格/库存口径混乱。

**种子数据**：

| activity_id | activity_name | strategy_id | begin_date_time | end_date_time | state |
|---|---|---|---|---|---|
| 100301 | 测试活动 | 100006 | 2024-03-09 10:15:10 | 2034-03-09 10:15:10 | open |
...
|---|---|---|---|---|
| 9011 | 100301 | 11101 | 20 | 0 |

> `stock_count_surplus = 0` 表示当前库存已耗尽（测试数据为库存扣减后的状态）。
...
**种子数据**（`marketing_02.raffle_activity_order_001`；`marketing_01.raffle_activity_order_001` 另有 Charlie 的大量下单流水）：
...
- `out_business_no` 是外部传入的业务防重ID，用于幂等控制——`uq_out_business_no` 唯一约束在数据库层面保证同一 `out_business_no` 只能创建一条订单
...
**职责**：将待发送的 MQ 消息落库，作为"消息表 + 定时扫描"分布式事务最终一致性方案的本地存储。每条记录对应一条待发出的 MQ 消息，除消息体外还冗余了 RabbitMQ 的完整投递参数（交换机、路由键、队列），扫描器按状态推进（`create` → `completed`/`fail`）。
...
|---|---|---|---|
| `id` | `int(11) unsigned` | PK, AUTO_INCREMENT | 自增主键 |
| `user_id` | `varchar(32)` | NOT NULL | 用户ID（分库路由键，与 `marketing_01`/`marketing_02` 分库键一致） |
| `message_id` | `varchar(32)` | NOT NULL | 消息唯一ID（与 `BaseEvent.EventMessage.id` 对齐，消费侧用作幂等键） |
| `exchange` | `varchar(64)` | NOT NULL, DEFAULT '' | RabbitMQ 交换机名；空串表示走 broker 默认交换机（与 `BaseEvent.exchange()` 对齐） |
| `routing_key` | `varchar(64)` | NOT NULL, DEFAULT '' | RabbitMQ 路由键；默认交换机下等于队列名（与 `BaseEvent.routingKey()` 对齐） |
| `queue` | `varchar(64)` | NULL | 目标队列名（与 `BaseEvent.queue()` 对齐）；fanout 等无路由场景可为 NULL，便于消费侧 `@RabbitListener` 定位 |
| `message` | `varchar(512)` | NOT NULL | 消息主体（JSON 序列化，即 `EventMessage` 结构） |
| `state` | `varchar(16)` | NOT NULL, DEFAULT 'create' | 任务状态：`create`（待发送）、`completed`（发送成功）、`fail`（发送失败） |
| `create_time` | `datetime` | NOT NULL | 创建时间 |
| `update_time` | `datetime` | NOT NULL | 更新时间 |

**索引**：`PRIMARY KEY (id)`、`KEY idx_state (state)`

**设计要点**：
- 与本地业务事务一起 commit，借助 MQ 中间件的事务消息表思路实现"最终一致性"
- 投递参数与 `BaseEvent` 事件规范一一对齐（`message_id`/`exchange`/`routing_key`/`queue`）：任务表不绑定单一 topic，任意事件的投递参数都可落库，由扫描器按记录内的参数直发 RabbitMQ
- `exchange` 为空串表示走 RabbitMQ 默认交换机，此时 `routing_key` 即队列名（如发奖事件 `SendAwardMessageEvent`：exchange=`''`、routing_key=queue=`send_award`）
- `message_id` 对齐 `EventMessage.id`（11 位随机数字），消费端可基于它做幂等，防重复消费
- 定时任务按 `state='create'` 扫描（走 `idx_state` 索引），发送 MQ 成功后更新为 `completed`；失败按重试策略更新为 `fail`
- 配合库存扣减、异步发奖等"先写本地、后发 MQ"的场景，避免消息丢失与业务回滚之间的不一致
...
> `tree_lock_2` / `tree_lock_3` 的节点结构与 `tree_lock_1` 相同，仅 `rule_lock.rule_value` 不同（分别为 `2`、`3`，即抽满 2/3 次后解锁）。
...
> `tree_lock_2` 的连线与 `tree_lock_1` 完全一致；`tree_lock_3` 的连线为 `rule_lock → rule_luck_award (ALLOW)`、`rule_lock → rule_luck_award (TAKE_OVER)`、`rule_stock → rule_luck_award (ALLOW)`——缺少 `rule_lock → rule_stock (ALLOW)` 且存在重复目标节点，疑似种子数据笔误，执行前建议修正。

---

### 4.18 `daily_behavior_rebate` — 日常行为返利配置表（公共库 `marketing`）

**职责**：定义"日常行为 → 返利"的运营配置。同一行为可配置多条返利（如签到同时返 sku 额度与积分），运行时按 `behavior_type` 查出该行为下全部开启的返利逐条入账。

| 字段名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `id` | `int(11) unsigned` | PK, AUTO_INCREMENT | 自增主键 |
| `behavior_type` | `varchar(16)` | NOT NULL | 行为类型：`sign`（签到）、`openai_pay`（支付） |
| `rebate_desc` | `varchar(128)` | NOT NULL | 返利描述 |
| `rebate_type` | `varchar(16)` | NOT NULL | 返利类型：`sku`（活动库存充值商品）、`integral`（用户活动积分） |
| `rebate_config` | `varchar(32)` | NOT NULL | 返利配置（`rebate_type=sku` 时为 sku 值、`integral` 时为积分值） |
| `state` | `varchar(12)` | NOT NULL | 状态：`open`（开启）、`close`（关闭） |
| `create_time` | `datetime` | NOT NULL | 创建时间 |
| `update_time` | `datetime` | NOT NULL | 更新时间 |

**索引**：`PRIMARY KEY (id)`、`KEY idx_behavior_type (behavior_type)`

**种子数据**：

| behavior_type | rebate_desc | rebate_type | rebate_config | state |
|---|---|---|---|---|
| `sign` | 签到返利-sku额度 | `sku` | `9011` | `open` |
| `sign` | 签到返利-积分 | `integral` | `10` | `open` |

**设计要点**：
- 一个 `behavior_type` 可对应多条返利配置：签到 = sku 额度（充值抽奖资格）+ 积分 双份返利
- `rebate_type` 决定 `rebate_config` 的语义：`sku` 时对应 `raffle_activity_sku.sku`，`integral` 时为入账积分值
- 运营配置表：运行时只读，可通过 `state` 随时开关某条返利，无需删数据
- 用户侧流水落在分库的 `user_behavior_rebate_order`（见 4.19），通过 `behavior_type` + `rebate_type` + `rebate_config` 与本表软关联（无外键）

---

### 4.19 `user_behavior_rebate_order` — 用户行为返利流水订单表（分库分表 `marketing_01`/`marketing_02`）

**职责**：记录用户每次行为返利的入账流水。返利流水与后续实际入账（sku 充值下单 / 积分入账）解耦：本表先落"返利已发放"的幂等凭据，再经 `task` 表 + MQ 异步驱动入账。
...
| `id` | `int(11) unsigned` | PK, AUTO_INCREMENT | 自增主键 |
| `user_id` | `varchar(32)` | NOT NULL | 用户ID（路由键） |
| `order_id` | `varchar(12)` | NOT NULL, UNIQUE | 订单ID |
| `behavior_type` | `varchar(16)` | NOT NULL | 行为类型（sign 签到、openai_pay 支付） |
| `rebate_desc` | `varchar(128)` | NOT NULL | 返利描述（从 `daily_behavior_rebate` 冗余） |
| `rebate_type` | `varchar(16)` | NOT NULL | 返利类型（sku 活动库存充值商品、integral 用户活动积分） |
| `rebate_config` | `varchar(32)` | NOT NULL | 返利配置【sku值，积分值】（从 `daily_behavior_rebate` 冗余） |
| `out_business_no` | `varchar(64)` | NOT NULL | 业务防重ID - 外部透传，方便查询使用 |
| `biz_id` | `varchar(128)` | NOT NULL, UNIQUE | 业务ID - 拼接的唯一值，拼接 `out_business_no + 自身枚举` |
| `create_time` | `datetime` | NOT NULL | 创建时间 |
| `update_time` | `datetime` | NOT NULL | 更新时间 |
...
**种子数据**（`marketing_01.user_behavior_rebate_order_001`）：

| user_id | order_id | behavior_type | rebate_desc | rebate_type | rebate_config | out_business_no | biz_id |
|---|---|---|---|---|---|---|---|
| `Charlie` | `833814327101` | `sign` | 签到返利 | `sku` | `9011` | （外部透传值） | `xiaofuge_sku_20240430` |
| `Charlie` | `509399206701` | `sign` | 签到返利-积分 | `integral` | `10` | （外部透传值） | `xiaofuge_integral_20240430` |

> 当前 SQL 文件中的种子数据 `INSERT` 语句尚未补齐 `out_business_no` 列，但表结构声明该列为 `NOT NULL`，重放脚本时需按拼接规则补齐 `out_business_no` 与 `biz_id` 的实际值。

**设计要点**：
- `biz_id` 是防重幂等键，由 `out_business_no + 自身枚举` 拼接（如 `out_business_no + "_sku"` / `out_business_no + "_integral"`），保证同一业务防重ID下同类型返利只能入账一次——这是"签到返利"按日/按次防刷的核心约束
- `order_id` 与 `biz_id` 双唯一约束：`order_id` 保证流水全局唯一，`biz_id` 保证业务语义幂等（重复请求直接命中唯一键冲突，拒绝重复入账）
- `out_business_no` 由外部调用方透传（与 `raffle_activity_order.out_business_no` 语义一致：业务防重ID），既用于 `biz_id` 拼接，也便于按业务方ID查询流水
- `rebate_desc` / `rebate_type` / `rebate_config` 从 `daily_behavior_rebate` 冗余拷贝，流水落库后运营改配置不影响历史记录
- 分库分表：`userId` → 哈希 → 分库（01/02）→ 分表（000~003），与其他用户流水表路由规则一致
- 与 `task` 表配合：返利流水落库后，`SendRebateMessageEvent` 返利消息经 `task` 表 + MQ 异步驱动 sku 充值下单 / 积分入账，靠 `task` 表做最终一致性补偿

---

### 4.20 `user_credit_account` — 用户积分账户表（分库 `marketing_01`/`marketing_02`，第 25 节新增）

**职责**：记录用户的积分资产（总积分/可用积分），由发奖消费链路（`SendAwardCustomer` → `AwardService.distributeAward` → `UserCreditRandomAward` / `UserCreditBlacklistAward` 等 `IDistributeAward` 实现）在发放积分类奖品（`award_key=user_credit_random` / `user_credit_blacklist`）时累加入账。

> DDL 已落地于 `marketing_01.sql` / `marketing_02.sql`（`marketing_01.sql:866`、`marketing_02.sql:455`），每个库 1 张表（不分表）。

| 字段名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `id` | `bigint(11) unsigned` | PK, AUTO_INCREMENT | 自增主键 |
| `user_id` | `varchar(32)` | NOT NULL | 用户ID（路由键） |
| `total_amount` | `decimal(10,2)` | NOT NULL | 总积分，展示用总账户值，记录一个人获得的总积分 |
| `available_amount` | `decimal(10,2)` | NOT NULL | 可用积分，每次扣减的值（发奖入账时与 `total_amount` 同加） |
| `account_status` | `varchar(8)` | NOT NULL | 账户状态：`open`（可用）/ `close`（冻结），由 `AccountStatusVO` 枚举维护 |
| `create_time` | `datetime` | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| `update_time` | `datetime` | NOT NULL, ON UPDATE | 更新时间 |

**索引**：`PRIMARY KEY (id)`；**当前 DDL 尚未对 `user_id` 加唯一索引**（见 Q10）

**账户状态枚举 `AccountStatusVO`**（`marketing-domain/.../award/model/valobj/AccountStatusVO.java`）：

| code | desc | 写入路径 |
|---|---|---|
| `open` | 开启 | `AwardRepository.saveGiveOutPrizesAggregate` 在账户首次开户时硬编码写入 |
| `close` | 冻结 | 预留状态，运行时暂无冻结流程切换 |

**发奖分发策略注册机制**（`IDistributeAward` + `award_key`）：

- 领域接口 `IDistributeAward`（`marketing-domain/.../award/service/distribute/IDistributeAward.java`）只有一个方法 `giveOutPrizes(DistributeAwardEntity)`，所有奖品发奖实现该接口；
- 发奖实现以 `@Component("award_key")` 注入 Spring 容器，bean 名 = `award.award_key`；
- `AwardService.distributeAward` 通过 `Map<String, IDistributeAward> distributeAwardMap`（Spring 自动按 bean 名组装）按 `award.award_key` 路由到具体实现，无需 if/else 分支；
- 当前已落地 `UserCreditRandomAward`（`@Component("user_credit_random")`）：解析 `award_config = "min,max"` → `BigDecimal.round(MathContext(3))` 生成随机积分 → 入账；
- 黑名单奖品 `user_credit_blacklist`（`award_config="1"`）暂未在 `service/distribute/impl/` 下独立成类，`award_config` 可由责任链/规则树透传 `awardConfig` 走相同的随机积分路径（`UserCreditRandomAward` 优先取 `distributeAwardEntity.getAwardConfig()`，未传时再 `repository.queryAwardConfig` 兜底）；
- **新增发奖实现**：`distribute/impl/` 新建 `XxxAward implements IDistributeAward`，类上 `@Component("award.award_key")` 即可被工厂自动注册，无需改服务层代码。

**写入路径**（`AwardRepository.saveGiveOutPrizesAggregate`，单事务）：
...
-- 影响 0 行（账户不存在）→ ② 插入新账户（total = available = 本次积分值，status='open'）
INSERT INTO user_credit_account(user_id, total_amount, available_amount, account_status, ...)
VALUES (#{userId}, #{amount}, #{amount}, 'open', ...);
```
...
-- 影响 0 行（已是 completed）→ 回滚，拦截重复发奖（MQ 至少一次投递下的消费幂等）
```

**`award_config` 透传机制**（第 25 节新增）：

- 抽奖抽象 `AbstractRaffleStrategy.buildRaffleAwardEntity` 透传 `awardConfig` 到 `RaffleAwardEntity`（责任链非默认接管 → `chainStrategyAwardVO.getAwardRuleValue()`；规则树 → `treeStrategyAwardVO.getAwardRuleValue()`）；
- `AwardService.saveUserAwardRecord` 把 `awardConfig` 写入 `SendAwardMessageEvent.SendAwardMessage.awardConfig`，随 MQ 消息下发；
- `SendAwardCustomer.listener` 把 `awardConfig` 透传到 `DistributeAwardEntity.awardConfig`；
- `UserCreditRandomAward.giveOutPrizes` 优先取 `distributeAwardEntity.getAwardConfig()`，未透传时再 `repository.queryAwardConfig(awardId)` 兜底——解决了黑名单奖品 101 走责任链接管时 `awardConfig` 始终为 null 的历史问题（详见详细设计文档 Q2）。

**设计要点**：
- **先 update 后 insert 的免查库开户**：入账时不先 SELECT 判断账户是否存在，直接 UPDATE，以影响行数 0/1 区分「累加」与「首次开户」，减少一次查询
- `total_amount` 与 `available_amount` 在入账时同加；两列分离是为后续「积分消耗/冻结」预留（消耗只扣 `available_amount`）
- `account_status` 由 `AccountStatusVO` 枚举（`open`/`close`）维护，当前 DDL 尚未对 `user_id` 加唯一索引——与 `raffle_activity_account.uq_user_id_activity_id` 的约束风格不一致，同一用户并发首次入账理论上存在重复开户风险（见已知问题 Q10）
- 积分额度 `decimal(10,2)` + 发奖侧 `BigDecimal`（3 位有效数字四舍五入），避免浮点误差
- 分库不分表：按 `userId` 路由到对应库，与 `user_award_record` 同库，保证「积分入账 + 中奖记录状态推进」可在同一事务内完成
...
raffle_activity (活动) ──── raffle_activity_sku (SKU)
  ...
  │     │ award_state=create → completed（发奖消费推进）
  │     ▼
  └── user_credit_account (用户积分账户)          ── 按 userId 分库
        MQ 发奖消费（award_key=user_credit_random）累加入账
...
daily_behavior_rebate (行为返利配置)             ── 公共库
  │ behavior_type / rebate_type / rebate_config（配置驱动，软关联）
  ▼
user_behavior_rebate_order (用户行为返利流水)    ── 按 userId 分库分表
  │ uq_biz_id 幂等（out_business_no + 自身枚举）
  │ uq_out_business_no 软关联（外部业务防重ID，允许多条流水对应同一 out_business_no）
  ▼
task (MQ 补偿) → sku 充值下单（raffle_activity_order）/ 积分入账
```
...
1. 策略装配（首次）：
   ...
   → 8 个奖品：101 概率 2%，102~108 各 3%（合计 23%，非完整概率）
   → 以最小概率 0.02 为刻度生成槽位，shuffle → Redis

2. 默认抽奖：
   随机索引 → HGET 概率表
   → 命中 awardId = 107（小霸王游戏机）

3. 规则树验证（awardId=107 挂载 tree_lock_1）：
   ┌──────────────────────────────────────────────────┐
...
2. 规则树验证（tree_lock_1）：
   ...
   │   WHERE strategy_id=100006 AND award_id=107        │
   │     AND award_count_surplus > 0                    │
   │   → 24 → 23（扣减成功）→ ALLOW                     │
   │                                                    │
   │ 按连线：rule_stock → rule_luck_award (ALLOW)       │
   │   → 最终 awardId = 107（保持原奖品）               │
   └──────────────────────────────────────────────────┘
   → 结果：awardId = 107（小霸王游戏机）
```
...
│      107, '小霸王游戏机', NOW(), 'create'                        │
│    )                                                             │
│    → uq_order_id 保证一次抽奖只产生一条中奖记录                   │
│                                                                  │
│ 5. 落库 MQ 任务（与上述业务同事务）                                │
│    INSERT INTO task (                                             │
│      user_id, message_id, exchange, routing_key, queue,           │
│      message, state                                               │
│    ) VALUES (                                                     │
│      'Charlie', '510923496701', '', 'send_award', 'send_award',  │
│      '{"id":"510923496701","timestamp":1724462400000,             │
│        "data":{"userId":"Charlie","awardId":107,                 │
│                "awardTitle":"小霸王游戏机"}}',                     │
│      'create'                                                     │
│    )                                                             │
│    → exchange='' 走默认交换机，routing_key 即队列名 send_award    │
│    → message_id 对齐 EventMessage.id，消费侧幂等                  │
│                                                                  │
...
│    SELECT * FROM task WHERE state='create'   （走 idx_state）     │
│    → 按记录内 exchange/routing_key 投递 RabbitMQ                  │
│      （exchange='' 默认交换机，routing_key='send_award'）          │
│    → 消费者按 award_key 路由发奖（openai_model → 开通 dall-e-3）│
...
结果：
  ...
  user_award_record_002: { order_id='202608240001', award_id=107, award_state='completed' }
...
> 对应第 22 节「用户行为返利入账」。领域层 `rebate` 包已提交（`BehaviorRebateService`、`BehaviorRebateAggregate`、`SendRebateMessageEvent` 等），仓储实现与触发入口尚未落地，本节描述规划流转。

```
用户 Charlie 于 2024-04-30 完成签到（behavior_type = 'sign'）

┌─────────────────────────────────────────────────────────────────┐
│ 1. 查询行为返利配置（公共库 marketing，走 idx_behavior_type）      │
│    SELECT * FROM daily_behavior_rebate                           │
│      WHERE behavior_type = 'sign' AND state = 'open'             │
│    → 2 条配置：                                                   │
│      { rebate_type='sku',      rebate_config='9011' }            │
│      { rebate_type='integral', rebate_config='10'   }            │
│                                                                  │
│ 2. 逐条生成返利流水（路由到分库分表）                              │
│    userId="Charlie" → 哈希 → marketing_01.user_behavior_rebate_order_001
│                                                                  │
│    对每条返利配置，拼装幂等键 biz_id：                             │
│      sku      → biz_id = "out_business_no" + "_sku"               │
│      integral → biz_id = "out_business_no" + "_integral"          │
│                                                                  │
│    INSERT INTO user_behavior_rebate_order_001 (                  │
│      user_id, order_id, behavior_type, rebate_desc,              │
│      rebate_type, rebate_config, out_business_no, biz_id         │
│    ) VALUES                                                      │
│      ('Charlie', '833814327101', 'sign', '签到返利',             │
│       'sku', '9011', 'out_business_no', 'out_business_no_sku'),  │
│      ('Charlie', '509399206701', 'sign', '签到返利-积分',        │
│       'integral', '10', 'out_business_no',                       │
│       'out_business_no_integral')                                │
│    → uq_biz_id 保证同一业务防重ID下同类型返利只入账一次           │
│    → 重复签到直接命中唯一键冲突，拒绝重复入账                     │
│                                                                  │
│ 3. 异步入账（SendRebateMessageEvent → task + MQ 补偿）           │
│    rebate_type='sku'      → 以 sku=9011 发起活动下单              │
│                             （复用场景一流程，充值抽奖资格）       │
│    rebate_type='integral' → 用户积分账户 +10                      │
└─────────────────────────────────────────────────────────────────┘

结果：
  user_behavior_rebate_order_001: 2 条流水（sku + integral，uq_biz_id 幂等）
  raffle_activity_account: sku 入账后额度 +1（经 raffle_activity_order + 账户初始化/累加）
  用户积分: +10
```
...
| **发奖入账**（第 25 节） | `userId` + `orderId` + `awardId` + `awardConfig` | `award`（award_key 路由 `IDistributeAward` 实现）→ `user_credit_account`（先 UPDATE 后 INSERT 入账） + `user_award_record.award_state='completed'` | 实际入账（积分/未来其他奖品类型） |

### 6.8 运营配置数据 vs 流程生成数据
...
| `task` | **流程生成** | 全部字段均为流程生成(`user_id`、`message_id`、`exchange`、`routing_key`、`queue`、`message`、`state`);`message_id` 与 MQ 投递参数由业务模块拼装,`state` 由调度器推进 |
...
| `daily_behavior_rebate` | **运营配置** | 全部业务字段(`behavior_type`、`rebate_desc`、`rebate_type`、`rebate_config`、`state`);`state` 控制返利开关,运行时只读 |
| `user_behavior_rebate_order` | **流程生成** | 全部字段由流程生成(`user_id`、`order_id`、`behavior_type`、`out_business_no`、`biz_id`);`out_business_no` 由外部调用方透传,`biz_id` 由 `out_business_no + 自身枚举` 拼接生成;`rebate_desc`、`rebate_type`、`rebate_config` 从 `daily_behavior_rebate` 冗余快照 |
| `user_credit_account`（第 25 节新增） | **流程生成** | 全部字段均由流程生成(`user_id`、`total_amount`、`available_amount`、`account_status`);`total_amount`/`available_amount` 由发奖消费者（`IDistributeAward` 实现）累加入账;`account_status` 首次开户时硬编码为 `open` |

> **公共字段说明**:`create_time`、`update_time` 在所有表中均由数据库/系统自动维护,不计入业务字段分类。

#### 6.8.2 数据流转场景中的字段来源标注

把 6.1 ~ 6.7 七个场景中,涉及的每一个字段的来源都明确标注,便于一眼区分"运营做了什么"和"系统做了什么"。
...
| task message_id exchange routing_key queue message | 流程生成 | 业务模块拼装,同事务提交 |
...
| 字段 | 来源 | 出处 |
|---|---|---|
| 返利配置 behavior_type rebate_desc rebate_type rebate_config | 运营配置 | daily_behavior_rebate 预配置,state 控制开关 |
| 返利流水 user_id | 流程生成 | 触发行为的用户,兼作分库路由键 |
| 返利流水 order_id | 流程生成 | 系统生成,uq_order_id 全局唯一 |
| 返利流水 out_business_no | 外部透传 | 调用方传入,既用于 biz_id 拼接,也方便按业务方ID查询 |
| 返利流水 biz_id | 流程生成 | out_business_no + 自身枚举 拼接,uq_biz_id 按业务防重ID防重 |
| 返利流水 rebate_desc rebate_type rebate_config | 运营配置 | 从 daily_behavior_rebate 冗余快照,落库后运营改配置不影响历史 |
| sku 入账活动订单 | 流程生成 | 复用场景一下单流程,充值抽奖资格 |
| 积分入账 | 流程生成 | MQ 消费侧执行,task 表补偿 |

**场景七:用户中奖发奖入账(6.9,第 25 节)**

| 字段 | 来源 | 出处 |
|---|---|---|
| award_key | 运营配置 | award 表,按 award_key 路由 `IDistributeAward` 实现 |
| award_config | 运营配置 + 流程透传 | award 表默认配置;`DistributeAwardEntity.awardConfig` 由责任链/规则树 `awardRuleValue` 透传覆盖 |
| 中奖记录 award_state | 流程生成 | create → completed,消费侧 UPDATE WHERE state='create' 推进 |
| 积分账户 user_id | 流程生成 | 中奖用户,兼作分库路由键 |
| 积分账户 total_amount / available_amount | 流程生成 | 发奖消费侧累加,首次开户 total = available = 本次积分值 |
| 积分账户 account_status | 流程生成 | 首次开户硬编码 `open`(枚举 `AccountStatusVO.open`),运行时无 close 切换路径 |
...
| 积分账户免查开户 | `user_credit_account` 入账采用「先 UPDATE WHERE user_id,影响 0 行则 INSERT」,避免先 SELECT 判断存在,减少一次查询;并发场景下需保证开户幂等——当前 DDL 缺少 `uq_user_id` 唯一索引,见已知问题 Q10 |
| 返利流水按业务幂等 | `user_behavior_rebate_order` 的 `uq_biz_id`(由 `out_business_no + 自身枚举` 拼接) 保证同一业务防重ID下同类型返利仅入账一次;`out_business_no` 由外部调用方透传,`rebate_desc`/`rebate_type`/`rebate_config` 是落库时的冗余快照,运营修改 `daily_behavior_rebate` 配置不影响历史流水 |

---

### 6.9 场景七:用户中奖发奖入账（第 25 节发奖服务实现）

> 本场景对应第 25 节「积分发奖服务实现」。`SendAwardCustomer` 已从「解析消息 + 日志」占位演进为完整闭环：`parse → AwardService.distributeAward(awardKey 路由) → IDistributeAward.giveOutPrizes → user_credit_account 先 UPDATE 后 INSERT + 中奖记录置 completed`。

```
用户 Charlie 中奖 awardId=101（user_credit_random），orderId=20260916000001

┌─────────────────────────────────────────────────────────────────┐
│ 1. 消费 send_award 队列（SendAwardCustomer.listener）            │
│    parse EventMessage<SendAwardMessage>                          │
│    → userId=Charlie, orderId=20260916000001, awardId=101,       │
│      awardConfig="1,100"（由责任链/规则树透传，见 §4.20）        │
│                                                                  │
│ 2. 构造 DistributeAwardEntity{userId, orderId, awardId,         │
│                               awardConfig}                      │
│    → 透传到 AwardService.distributeAward                        │
│                                                                  │
│ 3. 按 awardKey 路由 IDistributeAward 实现                        │
│    awardRepository.queryAwardKey(101) → "user_credit_random"    │
│    distributeAwardMap.get("user_credit_random")                 │
│      → UserCreditRandomAward                                     │
│                                                                  │
│ 4. UserCreditRandomAward.giveOutPrizes                           │
│    awardConfig="1,100" → split(Constants.SPLIT) → [1, 100]      │
│    creditAmount = generateRandom(1, 100)                        │
│      = min + Math.random() × (max − min), MathContext(3)        │
│    例：1 + 0.427 × 99 = 42.273 → round 3 位有效数字 → 42.3      │
│                                                                  │
│ 5. 构建 GiveOutPrizesAggregate{                                  │
│       userId,                                                    │
│       userAwardRecordEntity（awardState=complete）,              │
│       userCreditAwardEntity{userId, creditAmount=42.3}          │
│     }                                                            │
│    → repository.saveGiveOutPrizesAggregate（编程式事务）          │
│                                                                  │
│ 6. 单事务写入                                                     │
│    ① UPDATE user_credit_account SET                              │
│         total_amount = total_amount + 42.3,                      │
│         available_amount = available_amount + 42.3              │
│       WHERE user_id = 'Charlie'                                 │
│       → 影响 1 行（账户已存在,累加）                              │
│    ② UPDATE user_award_record SET award_state = 'completed'     │
│       WHERE user_id = 'Charlie' AND order_id = '...'            │
│         AND award_state = 'create'                              │
│       → 影响 1 行（推进状态）                                     │
│    任一影响 0 行 → status.setRollbackOnly() + INDEX_DUP/幂等拦截 │
│                                                                  │
│ 7. 重投兜底（UPDATE 影响 0 行场景）                                │
│    a. user_credit_account UPDATE 影响 0 → 账户不存在              │
│       → INSERT INTO user_credit_account(                         │
│           user_id, total_amount, available_amount,               │
│           account_status) VALUES (                              │
│           'Charlie', 42.3, 42.3, 'open')                        │
│    c. user_award_record UPDATE 影响 0 → 已是 completed            │
│       → rollback + warn「重复更新拦截」(MQ 至少一次投递幂等)     │
└─────────────────────────────────────────────────────────────────┘

结果：
  user_credit_account:        total_amount += 42.3, available_amount += 42.3
  user_award_record:          award_state = 'completed'
```

**发奖分发策略路由示例（`award_key` → `IDistributeAward`）**：

| award_key | 实现类 | 行为 | 状态 |
|---|---|---|---|
| `user_credit_random` | `UserCreditRandomAward` | 随机积分（解析 `award_config` 为 `min,max`） | **已落地** |
| `user_credit_blacklist` | `UserCreditRandomAward`（复用） | 黑名单奖品 100（`award_config="1"`），透传 `awardConfig` 走同一条随机积分路径 | **已复用** |
| `openai_use_count` | 待新增 | OpenAI 使用次数充值 | TODO |
| `openai_model` | 待新增 | OpenAI 模型开通 | TODO |

**关键设计**：
- **bean 名 = award_key**：`@Component("user_credit_random")` 与 `award.award_key` 对齐，新增奖品类型只需新建 `XxxAward implements IDistributeAward`，无需改服务层
- **`award_config` 透传**：抽象策略 → `RaffleAwardEntity.awardConfig` → `SendAwardMessage.awardConfig` → MQ → `DistributeAwardEntity.awardConfig` → `IDistributeAward` 优先使用，未透传时 `repository.queryAwardConfig` 兜底——解决了黑名单接管路径下 `awardConfig` 始终为 null 的历史问题
- **状态机推进方向**：`award_state` 只允许 `create → completed` 单向推进，反向操作直接 rollback，配合 `WHERE award_state='create'` 的条件 UPDATE 形成天然幂等
- **盲发奖品的兜底**：若 `award_key` 在 `distributeAwardMap` 中无对应实现，`AwardService.distributeAward` 抛 `RuntimeException("分发奖品，奖品" + awardKey + "对应的服务不存在")` → 消费者 rethrow → MQ 重试 3 次 → 死信，避免发奖静默丢失
...
| 分库分表 | `raffle_activity_order` / `user_raffle_order` / `user_award_record` / `user_behavior_rebate_order` 按 userId 分库分表（每库 4 张 `_000` ~ `_003`）；账户表（`raffle_activity_account` / `raffle_activity_account_day` / `raffle_activity_account_month`）与 `task` 表分库不分表 | 用户数据水平扩展，流水表数据量大需分表，账户表数据量小只分库 |
| 公共库独立 | 活动/策略/奖品/规则树等配置表不拆分 | 配置数据量小、读多写少，集中管理避免跨库查询 |
| 业务防重 | `out_business_no` 外部防重ID | 外部系统透传，确保同一业务请求只创建一条订单 |
| 活动-策略 1:1 约束 | `raffle_activity.uq_strategy_id` | 一个策略在同一时间只能被一个活动使用，避免多活动复用同一策略导致抽奖资格/库存口径混乱 |
| 返利流水双唯一约束 | `uq_order_id` + `uq_biz_id`(由 `out_business_no + 自身枚举` 拼接) | `uq_order_id` 保证流水全局唯一,`uq_biz_id` 保证同一业务防重ID下同类型返利仅入账一次,`out_business_no` 单独存储便于外部业务方按业务ID查询 |
...
| 发奖分发按 award_key 注册 | `IDistributeAward` 实现以 `@Component("award_key")` 注入,`AwardService.distributeAward` 按 `award.award_key` 路由 | 新增奖品类型只需新建 `XxxAward` 类,无需改服务层;支持多奖品类型发奖策略横向扩展 |
| 发奖入账免查开户 | `user_credit_account` 「先 UPDATE WHERE user_id,影响 0 行则 INSERT」单事务 | 避免先 SELECT 判断账户存在,减少一次查询;但当前 DDL 缺少 `uq_user_id` 唯一索引,同一用户并发首次入账理论上存在重复开户风险（已知问题 Q10） |
| award_config 透传 | 抽奖抽象 → `RaffleAwardEntity.awardConfig` → MQ 消息体 → `DistributeAwardEntity.awardConfig` → 发奖实现优先使用,未传时 `award` 表兜底 | 解决黑名单/权重接管的奖品在透传链路上 `award_config` 始终为 null 的历史问题（已知问题 Q2） |
| `idx_award_id` 索引名沿用 | 索引名 `idx_award_id` 但实际列指向 `strategy_id` | 历史迁移兼容,文档标注清楚即可;后续如重构建议改名 `idx_strategy_id` |
| **Q10 用户积分账户缺少唯一索引** | `user_credit_account` 当前 DDL 仅 PK 自增,未对 `user_id` 加唯一约束;发奖采用「先 UPDATE 影响 0 行 → INSERT」做幂等,极端并发场景下两个首次入账请求都看到 0 行时,可能产生重复开户;修复路径：补 `UNIQUE KEY uq_user_id (user_id)` → INSERT 因唯一键冲突进入 DuplicateKeyException → 调用方按业务规则合并或丢弃 | — |