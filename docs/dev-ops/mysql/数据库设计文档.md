...
| `biz_id` | `varchar(64)` | NOT NULL, UNIQUE | 业务ID - 拼接的唯一值（幂等键） |
| `create_time` | `datetime` | NOT NULL | 创建时间 |
| `update_time` | `datetime` | NOT NULL | 更新时间 |
...
| user_id | order_id | behavior_type | rebate_desc | rebate_type | rebate_config | biz_id |
|---|---|---|---|---|---|---|
| `Charlie` | `833814327101` | `sign` | 签到返利 | `sku` | `9011` | `xiaofuge_sku_20240430` |
| `Charlie` | `509399206701` | `sign` | 签到返利-积分 | `integral` | `10` | `xiaofuge_integral_20240430` |

**设计要点**：
- `biz_id` 是防重幂等键：由 `userId + rebate_type + 日期` 拼接（如 `Charlie_sku_20240430`），同一用户同一天同类型返利只能入账一次——这是"签到返利"按日防刷的核心约束；种子数据中的 `xiaofuge_sku_20240430` 为旧用户名遗留（前缀与 `user_id='Charlie'` 不符），重放脚本时建议按拼接规则修正
...
user_behavior_rebate_order (用户行为返利流水)    ── 按 userId 分库分表
  │ uq_biz_id 幂等（userId + rebate_type + 日期）
  ▼
...
│      sku      → biz_id = "Charlie_sku_20240430"                  │
│      integral → biz_id = "Charlie_integral_20240430"             │
│                                                                  │
│    INSERT INTO user_behavior_rebate_order_001 (                  │
│      user_id, order_id, behavior_type, rebate_desc,              │
│      rebate_type, rebate_config, biz_id                          │
│    ) VALUES                                                      │
│      ('Charlie', '833814327101', 'sign', '签到返利',             │
│       'sku', '9011', 'Charlie_sku_20240430'),                    │
│      ('Charlie', '509399206701', 'sign', '签到返利-积分',        │
│       'integral', '10', 'Charlie_integral_20240430')             │
│    → uq_biz_id 保证同一用户同一天同类型返利只入账一次              │
...
| `user_behavior_rebate_order` | **流程生成** | 全部字段由流程生成(`user_id`、`order_id`、`behavior_type`、`biz_id`);`rebate_desc`、`rebate_type`、`rebate_config` 从 `daily_behavior_rebate` 冗余快照 |
...
| 返利流水 biz_id | 流程生成 | userId + rebate_type + 日期 拼接,uq_biz_id 按日防重 |
...
| 返利流水按日幂等 | `user_behavior_rebate_order` 的 `uq_biz_id`(userId + rebate_type + 日期) 保证同一用户同日同类型返利仅入账一次;`rebate_desc`/`rebate_type`/`rebate_config` 是落库时的冗余快照,运营修改 `daily_behavior_rebate` 配置不影响历史流水 |
