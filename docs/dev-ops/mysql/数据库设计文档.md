...
marketing_01（分库 1，按 userId 哈希路由）          marketing_02（分库 2，按 userId 哈希路由）
  ...
  ├── user_behavior_rebate_order_000    ← 分表 0        ├── user_behavior_rebate_order_001*   ← 注*
  ├── user_behavior_rebate_order_001    ← 分表 1        ├── user_behavior_rebate_order_002*   ← 注*
  ├── user_behavior_rebate_order_002    ← 分表 2        ├── user_behavior_rebate_order_003*   ← 注*
  └── user_behavior_rebate_order_003    ← 分表 3        └── user_behavior_rebate_order_004*   ← 注*
```

> *注：`marketing_02.sql` 中 `user_behavior_rebate_order` 分表编号为 `_001` ~ `_004`（缺 `_000`、多 `_004`），与路由组件 `_000` ~ `_003` 的约定不符，疑似脚本笔误，执行前建议修正。
...
> DDL 已落地于 `marketing_01.sql`（`_000` ~ `_003`）与 `marketing_02.sql`。
> 注意：`marketing_02.sql` 中该表编号为 `_001` ~ `_004`（缺 `_000`、多 `_004`），与路由组件 `_000` ~ `_003` 的约定不符，疑似脚本笔误，执行前建议修正。
...
| user_id | order_id | behavior_type | rebate_type | rebate_config | biz_id |
|---|---|---|---|---|---|
| `xiaofuge` | `833814327101` | `sign` | `sku` | `9011` | `xiaofuge_sku_20240430` |
| `xiaofuge` | `509399206701` | `sign` | `integral` | `10` | `xiaofuge_integral_20240430` |

**设计要点**：
- `biz_id` 是防重幂等键：由 `userId + rebate_type + 日期` 拼接（如 `xiaofuge_sku_20240430`），同一用户同一天同类型返利只能入账一次——这是"签到返利"按日防刷的核心约束
...
- 与 `task` 表配合：返利流水落库后，sku 充值下单 / 积分入账等后续动作通过 MQ 异步驱动，靠 `task` 表做最终一致性补偿
...
> 对应第 22 节「用户行为返利入账」sql 设计（领域代码尚未落地，本节描述规划流转）。

```
用户 xiaofuge 于 2024-04-30 完成签到（behavior_type = 'sign'）
...
│                                                                  │
│ 2. 逐条生成返利流水（路由到分库分表）                              │
│    userId="xiaofuge" → 哈希 → marketing_01.user_behavior_rebate_order_001
│                                                                  │
│    对每条返利配置，拼装幂等键 biz_id：                             │
│      sku      → biz_id = "xiaofuge_sku_20240430"                 │
│      integral → biz_id = "xiaofuge_integral_20240430"            │
│                                                                  │
│    INSERT INTO user_behavior_rebate_order_001 (                  │
│      user_id, order_id, behavior_type, rebate_desc,              │
│      rebate_type, rebate_config, biz_id                          │
│    ) VALUES                                                      │
│      ('xiaofuge', '833814327101', 'sign', '签到返利-sku额度',    │
│       'sku', '9011', 'xiaofuge_sku_20240430'),                   │
│      ('xiaofuge', '509399206701', 'sign', '签到返利-积分',       │
│       'integral', '10', 'xiaofuge_integral_20240430')            │
│    → uq_biz_id 保证同一用户同一天同类型返利只入账一次              │
│    → 重复签到直接命中唯一键冲突，拒绝重复入账                     │
│                                                                  │
│ 3. 异步入账（经 task 表 + MQ 补偿，规划流转）                     │
