...
- **字符集**：`utf8mb4`
- **存储引擎**：`InnoDB`
- **总表数**：11 张（公共库）+ 40 张（分库物理表：4 种分表 × 4 张 × 2 库 + 4 种不分表 × 2 库），合计 51 张
...
- **分表数**：`raffle_activity_order` / `user_raffle_order` / `user_award_record` / `user_behavior_rebate_order` 每库 4 张（`_000` ~ `_003`），其余账户表与 `task` 表每库 1 张（不分表）
...
> `raffle_activity_order` / `user_raffle_order` / `user_award_record` / `user_behavior_rebate_order` 按 `userId` 哈希分到 `marketing_01` / `marketing_02`，每个库内再分 4 张表（`_000` ~ `_003`）；`raffle_activity_account` / `raffle_activity_account_day` / `raffle_activity_account_month` / `task` 分库但不分表。
...
把 6.1 ~ 6.6 六个场景中,涉及的每一个字段的来源都明确标注,便于一眼区分"运营做了什么"和"系统做了什么"。
